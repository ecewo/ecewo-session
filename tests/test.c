...
int test_session_value_set_get(void) {
  ...
  char *val1 = session_value_get(sess, "key1", NULL);
  char *val2 = session_value_get(sess, "key2", NULL);
  char *val3 = session_value_get(sess, "nonexistent", NULL);
...
int test_session_value_overwrite(void) {
  ...
  char *val = session_value_get(sess, "key", NULL);
  ASSERT_NOT_NULL(val);
...
int test_session_value_remove(void) {
  ...
  char *before = session_value_get(sess, "to_remove", NULL);
  ...
  char *after = session_value_get(sess, "to_remove", NULL);
  ASSERT_NULL(after);
...
int test_session_utf8_values(void) {
  ...
  session_value_set(sess, "turkish", "üç dört beş");
  session_value_set(sess, "emoji", "test 🎉");

  char *turkish = session_value_get(sess, "turkish", NULL);
  ASSERT_NOT_NULL(turkish);
  ASSERT_EQ_STR("üç dört beş", turkish);
  free(turkish);

  char *emoji = session_value_get(sess, "emoji", NULL);
